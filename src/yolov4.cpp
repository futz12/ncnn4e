...
extern "C" __declspec(dllexport) __yolov4 __stdcall yolov4_Init(const unsigned char *mem_param, int size_param, const unsigned char *mem_model, int size_model, int target_size, const bool use_vulkan)
{
    ...
    yolov4->target_size = target_size;
    return yolov4;
}

extern "C" int __declspec(dllexport) __stdcall yolov4_Deal(__yolov4 yolov4, const unsigned char *img_src, const int img_size, Object **ResList)
{
    if (yolov4 == NULL)
    {
    ...
    ncnn::Mat in = ncnn::Mat::from_pixels_resize(src_mat.data, ncnn::Mat::PIXEL_BGR2RGB, src_mat.cols, src_mat.rows, yolov4->target_size, yolov4->target_size);
